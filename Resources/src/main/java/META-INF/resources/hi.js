...
(d=e,c=e.getAttribute(b))});d&&(e.strictDi=null!==ee(d,"strict-di"),b(d,c?[c]:[],e))}function Ac(a,b,d){E(d)||(d={});d=T({strictDi:!1},d);var c=function(){a=C(a);if(a.injector()){var c=a[0]===W?"document":ta(a);throw Da("btstrpd",c.replace(/</,"&lt;").replace(/>/,"&gt;"));}b=b||[];b.unshift(["$provide",function(b){b.value("$rootElement",a)}]);d.debugInfoEnabled&&b.push(["$compileProvider",function(a){a.debugInfoEnabled(!0)}]);b.unshift("ng");c=fb(b,d.strictDi);c.invoke(["$rootScope","$rootElement",
    ...
    a(b)}):C=U;ia.elemenut=C;Cc=!0}}function sb(a,b,d){if(!a)throw Da("areq",b||"?",d||"required");return a}function Sa(a,b,d){d&&L(a)&&(a=a[a.length-1]);sb(D(a),b,"not a function, got "+(a&&"object"===typeof a?a.constructor.name||"Object":typeof a));return a}function Ta(a,b){if("hasOwnProperty"===a)throw Da("badname",b);}function Dc(a,b,d){if(!b)return a;b=b.split(".");for(var c,e=a,f=b.length,g=0;g<f;g++)c=b[g],a&&(a=(e=a)[c]);return!d&&D(a)?vc(e,a):a}function tb(a){for(var b=a[0],d=a[a.length-1],c,
...
Hi.controllerReady=false;


//Configurar o hi
Hi.config = function(params){

    if(params.enableAjaxLoader){


        Hi.UI.loader = params.enableAjaxLoader;
...
Hi.getPlugin = function(name){


    if(Hi.plugins.hasOwnProperty(name)){


        return Hi.plugins[name];
...
Hi.getPlugins = function(){

    var setters = {};

    for(var pluginName in Hi.plugins){

        setters['$'+pluginName] = Hi.plugins[pluginName].$function();
...
Hi.pluginsFireEvent = function(name){

    for(var p in Hi.plugins){

        var plugin = Hi.plugins[p];
...
Hi.Plugin = function(name,constructor,core,pluginEventsHandlers,singleInstance){

    Hi.messages.log('creating plugin');
    ...
    this.singleInstance = this.singleInstance;
    ...
    this.$function = function(){
        ...
        if(singleInstance){
            ...
            if(Hi.pluginsInstances.hasOwnProperty(this.name)){
                ...
                //alert('new core');
                this.core['instance'] = Hi.pluginsInstances[this.name];
    ...
    this.construct = function(params){
        ...
        Hi.pluginsInstances[this.name] = instance;
    ...
    if(Hi.plugins.name){
        ...
        Hi.messages.warn('Hi plugin duplication : '+name);
    ...
    Hi.plugins[name] = this;

    //Os plugins ainda nao foram carregados
    if(!Hi.UI.pluginsLoaded){
...
Hi.angular.js = function(definitions){
        ...
        if(definition_name.valueOf()==='directives'&&typeof definition_value ==='object'){
            ...
            for(var directive_name in directives_list){
                ...
                if(typeof directive_function === 'function'){
                    ...
                    //Cria a directiva
                    Hi.angular.module.directive(directive_name,directive_function);
            ...
            for(var filter_name in filters_list){
                ...
                //Cria o filtro
                Hi.angular.module.filter(filter_name,filter_function);
            ...
            definition_value.apply(angular,Hi.angular.module);
...
//Inicializa a cache
Hi.UI.cache.initialize = function(){
    try {
...
Hi.UI.prepareView = function(route_name_or_object){
    ...
    if(typeof route_name_or_object ==='string'){
        ...
        route = Hi.routes.get(route_name_or_object);
    ...
    if(route){
        ...
        var path = Hi.Internal.generateViewPath(route.module,route.controller, route.action);


        var url = Hi.Internal.getURL(route);

        //Esta rota ja foi cacheada
        if(Hi.UI.cache.stores(url)){
        ...
        $.get(url,function(reponse){
            ...
            try{
                ...
                //Coloca na cache
                Hi.UI.cache.storeView(url,html);
...
Hi.UI.cache.storeView = function(path,markup){

    try {

        //Session storage disponivel
        if (sessionStorage && Hi.UI.cache.on) {
...
Hi.UI.cache.stores = function(path){
...
Hi.UI.cache.fetch = function(path){
...
//Scope actual
_ = false;
Hi.Internal.next$this = {};

Hi.Internal.$thisInclude = function(name,value){

    Hi.Internal.next$this[name] = value;


};

Hi.Internal.$thisIncludesDelete = function(){

    Hi.Internal.next$this = {};

};

Hi.Internal.$thisApply = function(){

    for(var key in Hi.Internal.next$this){

        var value = Hi.Internal.next$this[key];

        _[key] = value;

    }

    Hi.Internal.$thisIncludesDelete();

};


//Aplicar o scope
Hi.flush = function(callback){
...
Hi.UI.reload_title = true;

//Usando o titulo de uma view
Hi.UI.view_title=false;

//Definir o titulo da pagina
Hi.UI.setTitle = function(title){

    $("title").html(title);

};

Hi.UI.partials_loader = false;

Hi.UI.setPartialsLoader = function(loader){

    Hi.UI.partials_loader = loader;

};


Hi.goto = function(element_id,name){
    var route_command = "Hi.route('"+name+"');";
    $("#"+element_id).attr('onClick',route_command);
};

//Faz bind do clique ou de outro evento de um elemento a uma rota
Hi.bind = function(element_id,name,eventName){
...
Hi.UI.uimaster=false;

//Closures javascript de views
Hi.UI.viewsClosures={};

//Closures que estao a espera que seus parentes sejam declarados
Hi.UI.viewsClosuresChildsWaiting = {};

Hi.UI.viewsClosuresChildsDeclared = new Array();

Hi.UI.viewsClosuresDeclared = new Array();

//Handlers para eventos de contactos
Hi.UI.viewsEventsHandlers={};

Hi.UI.nextViewContext=false;
Hi.UI.nextViewPath=false;
Hi.UI.nextDialogContext=false;

Hi.UI.master__ = {};

//Regista o controlador do template raiz
Hi.UI.master = function(master){
    Hi.UI.master__=master;
};
...
Hi.UI.master = function(master){
    ...
    Hi.UI.fireMasterLoadEvent();


};

Hi.UI.pluginsLoaded = false;
Hi.UI.pluginsOnce = function(){

    if(Hi.UI.uimaster.$plugins){

        //Constroi instancias para cada um
        for(var pluginName in Hi.UI.uimaster.$plugins){


            var material = Hi.UI.uimaster.$plugins[pluginName];


            var plugin = Hi.getPlugin(pluginName);

            if(plugin){

                Hi.messages.log('plugin found '+pluginName);
                ...
                Hi.messages.log('Plugin missing : '+pluginName);
    ...
    Hi.pluginsLoaded = true;


};

Hi.ang = {modules:[],run:false};
...
Hi.template = function(master){
    ...
    //OLD VERSION
    //Hi.UI.uimaster=master;

    jQuery.extend(master,Hi.UI.master__);
    Hi.UI.uimaster=master;
...
Hi.templateContext = false;

//Quando o template for carregado
Hi.UI.fireMasterLoadEvent = function(){


    //Pega o titulo da aplicacao
    Hi.UI.title = $("title").html();
    ...
    $(function(){
        ...
        if(Hi.UI.uimaster){
            ...
            if(Hi.UI.uimaster.hasOwnProperty('init')){
...
//Quando o template for carregado
Hi.UI.fireMasterConfigEvent = function(params){
    ...
    if(Hi.UI.uimaster){
        ...
        if(Hi.UI.uimaster.hasOwnProperty('config')){

            Hi.UI.uimaster.config(params);

        }


        if(Hi.UI.uimaster.hasOwnProperty('preLoad')){



            Hi.UI.uimaster.preLoad();
...
Hi.Internal.setNextViewPath = function(module_name,model,action){


    var nvp = {module_name:module_name,model:model,controller:model,action:action};
    Hi.Internal.nextViewPath= nvp;

    var new_path = {module_name:module_name,model:model,controller:model,action:action};
    var path_with_params = Hi.Internal.getURL(new_path);

};

Hi.Internal.generateViewPath = function(module_name,model,action){
    var view_path=false;
...
Hi.Internal.getClosures = function(module_name,model,action){

    var view_path = Hi.Internal.generateViewPath(module_name,model,action);

    if(Hi.UI.viewsClosures.hasOwnProperty(view_path)){

        //Obtem os closures da view
        var closures_array = Hi.UI.viewsClosures[view_path];
...
Hi.Internal.pushViewClosure= function(module_name,model,action,closure){
    var view_path = Hi.Internal.generateViewPath(module_name,model,action);
    ...
    if(Hi.UI.viewsClosures.hasOwnProperty(view_path)){
        ...
        Hi.UI.viewsClosures[view_path]=closures_array;
...
Hi.Internal.closureWaiting = function(view_route,closure_declaration){


    var path = Hi.Internal.generateViewPath(view_route.module_name,view_route.model,view_route.action);

    var closures_waiting_array = false;


    //Ja existem closures a espera da declaracao do mesmo closure
    if(Hi.UI.viewsClosuresChildsWaiting.hasOwnProperty(path)){

        closures_waiting_array = Hi.UI.viewsClosuresChildsWaiting[path];
        ...
        Hi.UI.viewsClosuresChildsWaiting[path] = closures_waiting_array;
...
Hi.Internal.declareAllUndeclared = function(){

    for(var path in Hi.UI.viewsClosuresChildsWaiting){


        var closures_waiting_array = Hi.UI.viewsClosuresChildsWaiting[path];

        if(Hi.UI.viewsClosuresChildsDeclared.indexOf(path)==-1){
    ...
    Hi.UI.viewsClosuresChildsDeclared.push(path);


};

//Um closure que estava sendo esperado, chama os closures em espera
Hi.Internal.declareClosureChilds = function(view_route){
...
Hi.Internal.viewController=false;

Hi.Internal.startedup=false;

Hi.Internal.prepareNextViewContext = function(dialog,context_variables,markup,startup){
    ...
    if(startup){
        ...
        if(Hi.Internal.startedup){

            return;

        }else{

            Hi.Internal.startedup = true;
    ...
    if(dialog){
        ...
        Hi.UI.ngApp.controller('dialog_controller',function($scope){
        ...
        Hi.UI.nextDialogContext=context;

    }else{


        //Ainda nao existe controllador
        if(!Hi.Internal.viewController){


            //Cria o controlador
            Hi.Internal.createController(dialog,context_variables,markup);


        }else{


            Hi.Internal.callMeBack(dialog,context_variables,markup);
...
Hi.Internal.callMeBack = function(dialog,context_variables,markup){
    ...
    if(context_variables.$commands){
        ...
        for(var commandName in commands){
            ...
            if(commandName!='removeVal'){
                ...
                if(Hi.commands.run(commandName,commandParams)){
    ...
    if(markup){
        ...
        if(viewToLoad.extend){
            ...
            var child_path = Hi.Internal.getURL(child_route);


            if(Hi.UI.locker.isLocked(child_path)){

                //Child path is locked
                Hi.UI.locker.waitFor(child_path,'ntr',function(){


                    Hi.Internal.callMeBack(dialog,context_variables);
            ...
            //View is cached
            if(Hi.UI.cache.stores(child_path)){

                //Fetches the view from cache
                html = Hi.UI.cache.fetch(child_path);
    ...
    //Pega o controlador
    var context=Hi.Internal.viewController;

    Hi.UI.closeView();

    //Limpa o contexto
    Hi.Internal.cleanContext(context);



    //Faz bind dos parametros
    var load = Hi.Internal.bindParamsAndClosures(context,context_variables);
    ...
    if(load.preLoad){
        ...
        html = Hi.Internal.preLoad(load.preLoad,html,context);
    ...
    Hi.pluginsFireEvent('preLoad');


    //PostLoads
    if(load.postLoad){

        html = Hi.Internal.postLoad(load.postLoad,html);

    }


    //Dispara o postLoad para os plugins
    Hi.pluginsFireEvent('postLoad');
    ...
    if(context.hasOwnProperty('_title')){
        ...
        if(Hi.UI.reload_title===true){


            //define o titulo default
            Hi.UI.setTitle(Hi.UI.title);
    ...
    if(html){
        ...
        if(load.postLoad){
            ...
            Hi.Internal.runPostLoad(load.postLoad,context);
    ...
    Hi.UI.nextViewContext=context;
    //Hi.UI.setLocation('demo/home',route_object);



};

Hi.solve = function(func,toInject){

    var $injector = angular.injector(['ng','hi']);
...
Hi.Internal.createController = function(dialog,context_variables,markup){

    Hi.UI.ngApp.controller('view_controller',function($scope,$compile,$rootScope){
        ...
        if(Hi.UI.uimaster){
            ...
            for(template_variable_name in Hi.UI.uimaster){

                var template_variable_value = Hi.UI.uimaster[template_variable_name];
                ...
                if(private_index!=-1){
                    ...
                    //Variavel publica
                    __[template_variable_name] = template_variable_value;
                }
        ...
        Hi.Internal.configureContext($scope,true);
        Hi.Internal.viewController=$scope;

        if(!Hi.controllerReady){


            Hi.controllerReady=true;
...
Hi.Internal.started_up = false;

Hi.Internal.configureContext = function(context,create_controller){
        ...
        if(create_controller){
            ...
            after_apply = function(){
                ...
                if(__.afterViewLoad){
                    ...
                    if(typeof __.afterViewLoad ==='function' ){

                        if(typeof Hi.Internal.nextViewPath!="undefined"){

                            var resolved_route = Hi.routes.resolve(Hi.Internal.nextViewPath);
                            __.afterViewLoad.apply(_,[resolved_route]);
            ...
            //Startup
            if(!Hi.Internal.started_up){


                Hi.Internal.started_up = true;

                //Connect the push websocket
                Hi.Internal.Push.init();
                ...
                if(__.startup){
                    ...
                    if(typeof __.startup ==='function' ){
                        ...
                        if(typeof Hi.Internal.nextViewPath!="undefined"){
                            ...
                            var resolved_route = Hi.routes.resolve(Hi.Internal.nextViewPath);
                            __.startup.apply(__,[resolved_route]);
            ...
            context.$applyAsync(after_apply);

        }

        Hi.UI.putView(compiled);
...
Hi.Internal.cleanContext = function(context){
...
Hi.commands.set = function(key,callback){

    Hi.commands.all[key]=callback;

};


//Comando para recarregar a pagina
Hi.commands.set('$reload',function(url){
...
Hi.commands.set('$url',function(url){

    Hi.UI.setLocation(url,{});

});

//Comando para mudar o contexto
Hi.commands.set('$context',function(context){
...
Hi.commands.run = function(key,params){


    if(Hi.commands.all.hasOwnProperty(key)){

        var command = Hi.commands.all[key];
...
Hi.command = Hi.commands.run;

Hi.Internal.bindParamsAndClosures = function(context,context_variables){
    ...
    if(Hi.Internal.nextViewPath){


        var module_name= Hi.Internal.nextViewPath.module_name;
        var model = Hi.Internal.nextViewPath.model;
        var action = Hi.Internal.nextViewPath.action;
        var postLoad = false;
        var preLoad = false;
        ...
        /*
         var server_group =Hi.getServerGroup({"module":module_name,"model":model,"action":action});
        ...
        var closures=Hi.Internal.getClosures(module_name,model,action);
        ...
        if(closures){
            ...
            for(var closure_index in closures){
                ...
                for(var closure_name in closures){
                    ...
                    if(closure_name==='window'){
                        ...
                        Hi.Internal.setWindowEvents(closures[closure_name]);

                    }

                    if(closure_name==='document'){

                        Hi.Internal.setDocumentEvents(closures[closure_name]);
            ...
            console.log("Closures NOT found for "+model+"/"+action);
        ...
        console.log("Next view path not found");
...
Hi.Internal.setWindowEvents = function(eventsHandlers){
...
Hi.Internal.setDocumentEvents = function(eventsHandlers){
...
Hi.UI.loaderCompleted = function(){
...
Hi.php = {};
Hi.php.getInterface = function(route){

    var route_ = Hi.routes.resolve(route);

    return Hi.getServerGroup(route_);

};

Hi.UI.Interface = function(route){

    var route_ = Hi.routes.resolve(route);

    var closure = {};

    var closures = Hi.Internal.getClosures(route_.module,route_.controller,route_.action);
    ...
    this.php = Hi.getServerGroup(route_);

};

Hi.UI.loaderLoading = function(){
...
Hi.UI.closeView = function(){
    ...
    if(__){
        ...
        //A view declarou um evento onClose
        if(__.hasOwnProperty('$onClose')){

            //Executa o onClose
            var onClose = __['$onClose'];
...
Hi.UI.displayView = function(markup){


    $("#view_content").html(markup);
    $('#view_content').animate({scrollTop:0}, 'slow');
    Hi.UI.loaderCompleted();
    //Hi.trigger_ready();
};

Hi.UI.putView = function(markup){
...
Hi.UI.displayDialog = function(markup){
    $("#view_content").html(markup);
};

//Obtem o contexto da visao actual
Hi.UI.getContext = function(){

};


Hi.dialog = function(route_name){

    if(route_name){

        var route = Hi.routes.get(route_name);
        route['dialog']=true;
...
Hi.routes.last =  false;

Hi.history = [];

Hi.logRoute = function(url){

    Hi.history.push(url);
    return Hi.history.length -1;

};


Hi.isGoingBack = false;

Hi.getLog = function(url){

    if(Hi.history.length>0){

        var theUrl = Hi.history[0];
        Hi.history.removeVal(theUrl);
        return theUrl;
...
Hi.getLog = function(url){
    ...
    if(Hi.history.hasOwnProperty(url)){

        return Hi.history[url];
...
Hi.routes.new = function(name,object){

    if(!Hi.routes.hasOwnProperty(name)){
        Hi.routes.all[name]=object;
        return object;
    }
...
Hi.startFrom = function(route){

    Hi.routes['entrance_route']=route;
...
Hi.reloadView = function(){


    //Ultima rota encontrada
    if(Hi.routes.last){

        if(Hi.routes.last.name){

            var name = Hi.routes.last.name;

            if(Hi.routes.last.params){

                var params = Hi.routes.last.params;
                Hi.route(name,params);
...
Hi.preparing = false;
Hi.on_ready_callbacks = [];
Hi.on_ready = function(func){

    Hi.on_ready_callbacks.push(func);


};


Hi.trigger_ready = function(){

    Hi.on_ready_callbacks.forEach(function(item,index){
...
Hi.route = function(route_name_or_object,getParams){

    //Hi is preparing the this context object
    Hi.preparing = true;

    //Hi.routes.last = {name:route_name_or_object,params:params};
    Hi.routes.last = {name:route_name_or_object};

    var route_object = Hi.routes.resolve(route_name_or_object);
    ...
    */
    ...
    //Path da View
    var clean_path = Hi.Internal.getURL(route_object);


    if(Hi.UI.loaderSettings.hasOwnProperty(clean_path)){

        Hi.messages.warn('loader settings found for: '+clean_path);
        ...
        Hi.messages.info('loader settings not found for path: '+clean_path);
        ...
        if(!Hi.UI.disableLoader){
            ...
            if(Hi.UI.ajaxLoader.current){
                ...
                Hi.messages.log('showing loader by default');
    ...
    var path = Hi.Internal.getURL(route_object);
    ...
    if(!route_object.dialog){
        ...
        if(Hi.UI.cache.stores(path)){

            cached_view = Hi.UI.cache.fetch(path);
            server_directives = {'Ignore-View':'true'};

        }

        if(Hi.wasControllerLoaded(route_object.controller,route_object.action)){
        ...
        Hi.Internal.getRoute(route_object,function(server_response){
            ...
            if(server_response.controller) {
                ...
                Hi.Internal.setNextClosureInformation(false, route_object.controller, route_object.action);
                eval(server_response.controller);
                Hi.setLoadedController(route_object.controller, route_object.action);

            }


            //A view nao esta na cache
            if(!Hi.UI.cache.stores(clean_path)){

                Hi.UI.cache.storeView(clean_path,markup);
            ...
            if(Hi.UI.cache.stores(clean_path)){
                ...
                markup = Hi.UI.cache.fetch(clean_path);
            ...
            Hi.Internal.setNextViewPath(module_name,controller,action);
            var routeIndex = Hi.logRoute(path);
            Hi.UI.setLocation(path,JSON.stringify({index:routeIndex}));


            Hi.Internal.prepareNextViewContext(route_object.dialog,context_variables,markup);
...
Hi.routeBack = function(route_name_or_object,getParams){

    Hi.isGoingBack = true;
    Hi.route(route_name_or_object,getParams);


};

//Redirecciona o usuario para determinada rota
Hi.redirect = Hi.route;

Hi.ajaxify = Hi.route;


Hi.Internal.encodeGetParams = function(params){
...
Hi.Internal.encodeParams= function(params){
    var params_uri="";
...
Hi.Internal.eventURL = function(route,name){
    ...
    if(App.context){
        ...
        url = Hi.Internal.getURL(route,App.context+"/$event/"+name);

    }else{

        url = Hi.Internal.getURL(route,"false/$event/"+name);
...
Hi.Internal.getURL= function(route,covw) {
    ...
    if (route.params) {
        route_url = route_url + "/" + Hi.Internal.encodeParams(route.params);
    }
    ...
    if(route.get){
        ...
        if(typeof route.get ==="string"){
            ...
            var get_params = Hi.Internal.encodeGetParams(route.get);
...
Hi.UI.currentPage=false;
Hi.UI.currentRoute = false;

Hi.UI.setLocation = function (location,route){

    Hi.UI.currentPage=location;
    Hi.UI.currentRoute = route;
...
Hi.UI.resetLocation = function(){

    if(Hi.UI.currentPage){


        window.history.replaceState(Hi.UI.currentRoute,Hi.UI.currentPage+Math.random(),Hi.UI.currentPage);
...
Hi.UI.isSameRoute = function(url){

    return Hi.UI.currentPage===url;
...
Hi.Internal.getRoute = function(route,callback,server_directives){

    if(route){

        //Obtem a url da rota
        var route_url = Hi.Internal.getURL(route);
...
Hi.UI.setApp = function(app){
    Hi.UI.ngApp=app;
    Hi.angular.module=app;
...
Hi.UI.Controller = {};
Hi.UI.cpstyles = {};
Hi.UI.readyControllersHandlers={};


Hi.UI.partialReady = function(name,callback){



    if(Hi.UI.partialsInstances.hasOwnProperty(name)){

        callback(Hi.UI.partialsInstances[name]);

        return;

    }

    Hi.UI.readyControllersHandlers[name]=callback;
};

Hi.UI.componentReady = Hi.UI.partialReady;


Hi.Internal.fireControllerReady = function(name,controller){
    if(Hi.UI.readyControllersHandlers.hasOwnProperty(name)){
        Hi.UI.readyControllersHandlers[name](controller);
    }
};

Hi.Internal.nextClosureInformation = {};
Hi.Internal.setNextClosureInformation = function(module_name,model,action){
    Hi.Internal.nextClosureInformation = {module_name:module_name,model:model,action:action};
};

//Handlers para eventos da view
Hi.UI.onLoad = function(view_name,callback){

    var model_name=Hi.Internal.nextClosureInformation.model;
    var handlers_path = Hi.Internal.nextClosureInformation.module_name+'_'+model_name+'_'+view_name;


    if(Hi.UI.viewsEventsHandlers.hasOwnProperty(handlers_path)){

        var handlers = Hi.UI.viewsEventsHandlers[handlers_path];
        ...
        Hi.UI.viewsEventsHandlers[handlers_path] = handlers;

    }
};

Hi.Internal.nextPartialModule = false;
Hi.Internal.nextPartialName = false;
Hi.Internal.setNextPartialInformation = function(module_name,name){
    Hi.Internal.nextPartialModule=module_name;
    Hi.Internal.nextPartialName = name;
};

//Quando o controlador de um elemento estiver pronto
Hi.UI.whenReady = function(name,controller){
    return Hi.Internal.fireControllerReady(name,controller);
};


//Loader desabilitado por default
Hi.UI.disableLoader = false;

//Definicoees de loaders
Hi.UI.loaderSettings = {};


//Cria um closure javascript para uma determinada view
Hi.view = function(closure){


//Obtem informacao sobre o closure que esta sendo registrado
    var closure_information = Hi.Internal.nextClosureInformation;

//Resolved route
    var route = Hi.routes.resolve(closure_information);

//Rota correspondente a esta view
    closure.$route = route;

//Route path
    var route_path = Hi.Internal.getURL(route);

//Definicoes de loaders encontradas
    if(closure.hasOwnProperty('loader')){

        //Arquiva as definicoes de loaders
        Hi.UI.loaderSettings[route_path] = {};
...
//Regista o closure
    Hi.Internal.pushViewClosure(closure_information.module_name,closure_information.model,closure_information.action,closure);

//Gera o path do closure
    var path = Hi.Internal.generateViewPath(closure_information.module_name,closure_information.model,closure_information.action);

//Considera este closure declarado
    Hi.UI.viewsClosuresDeclared.push(path);


//Declara todos closures filhos deste
    Hi.Internal.declareClosureChilds(closure_information);
...
Hi.Internal.buildClosureChild = function(parent_closure,child){
...
Hi.UI.getTemplateHTML = function(name){


};

//Obtem o HTML de uma view
Hi.UI.getViewHTML = function(module_name,model,controller){
...
Hi.UI.locker.nodes = {};
Hi.UI.locker.waiters = {};

//Espera que uma view seja debloqueada
Hi.UI.locker.waitFor = function(path,waiterID,unlockWaiter){

    if(Hi.UI.locker.isLocked(path)){


        //Ja existem waiters
        if(Hi.UI.locker.waiters.hasOwnProperty(path)){

            var waiters = Hi.UI.locker.waiters[path];
            ...
            var waiters = {};
            waiters[waiterID]=unlockWaiter;
            Hi.UI.locker.waiters[path]=waiters;

        }

    }else{

        Hi.messages.error('Locker: '+path+" IS NOT LOCKED");
...
//Bloqueia uma view
Hi.UI.locker.lock = function(path){
    ...
    if(!Hi.UI.locker.isLocked(path)){
        ...
        var lock_node = {};
        lock_node.path = path;
        Hi.UI.locker.nodes[path] = lock_node;
...
Hi.UI.locker.isLocked = function(path){

    return Hi.UI.locker.nodes.hasOwnProperty(path);

};


//Desbloqueia uma view
Hi.UI.locker.unlock = function(path){

    //Se a view estiver bloqueada
    if(Hi.UI.locker.isLocked(path)){

        //Carrega o no de bloqueio
        var lock_node = Hi.UI.locker.nodes[path];
        ...
        if(Hi.UI.locker.waiters.hasOwnProperty(path)){
            ...
            delete Hi.UI.locker.waiters[path];
...
Hi.routes.toSlashes = function(route){

    return Hi.Internal.getURL(route);

};

//Resolve uma rota
Hi.routes.resolve = function(param){
...
Hi.UI.dumpView = function(route,dumpCallback){

    var route_object = Hi.routes.resolve(route);

    if(route_object){

        var url = Hi.Internal.getURL(route_object,'@view_dump');
...
Hi.Internal.compileExtendedView = function(parent_route,child_route,parent_view,child_view){


    var parent_path = Hi.Internal.getURL(parent_route);
    var child_path = Hi.Internal.getURL(child_route);
    ...
    Hi.UI.cache.storeView(child_path,final_html);
...
Hi.UI.extend = function(parent_view_name,closure){

    var parent = Hi.Internal.nextClosureInformation;

    var child = JSON.parse(JSON.stringify(parent));
    child.action = parent_view_name;

    var child_route = Hi.routes.resolve(parent);
    var parent_route = Hi.routes.resolve(child);

    var parent_path = Hi.Internal.getURL(parent_route);
    var child_path = Hi.Internal.getURL(child_route);

    //Bloqueia as duas views
    Hi.UI.locker.lock(parent_path);
    Hi.UI.locker.lock(child_path);

    //Faz dump da parent view
    Hi.UI.dumpView(child,function(parent_view){


        //Coloca a parent view na cache
        Hi.UI.cache.storeView(parent_path,parent_view);


        //Desbloqueia a view
        Hi.UI.locker.unlock(parent_path);

        //Faz dump da child view
        Hi.UI.dumpView(parent,function(child_view){


            //Compila a view extendida
            Hi.Internal.compileExtendedView(parent_route,child_route,parent_view,child_view);

            //Desloqueia a view
            Hi.UI.locker.unlock(child_path);
    ...
    var path = Hi.Internal.generateViewPath(child.module_name,child.model,child.action);



    //Ja foi declarado om closure pai deste closure
    if(Hi.UI.viewsClosuresDeclared.indexOf(path)!==-1){

        parent_closure = Hi.Internal.getClosures(child.module_name,child.model,child.action);
        Hi.Internal.nextClosureInformation = parent;
        child_closure = Hi.Internal.buildClosureChild(parent_closure,closure);
        Hi.view(child_closure);
        ...
        Hi.Internal.closureWaiting(child,function(){

            Hi.Internal.nextClosureInformation=parent;

            Hi.UI.extend(parent_view_name,closure);
...
Hi.UI.partial=function(options) {
    var name = Hi.Internal.nextPartialName;
    var module_name = Hi.Internal.nextPartialModule;
    ...
    var cpcontroller = function(el){
        ...
        this.getLoaderTemplate  = function(){
            ...
            if( typeof Hi.UI.partials_loader=='function'){


                return Hi.UI.partials_loader();
        ...
        this.styleInterface = function(){
            ...
            for(var style_name in this.styles){
                ...
                new Hi.Internal.StylePropertiesSetter(this,style_name,properties,this.element);
    ...
    Hi.UI.controllers[name]=cpcontroller;
    Hi.Internal.createDirectives(module_name,name,styles,cpcontroller);
...
Hi.Internal.StylePropertiesSetter = function(controller,style_name,properties) {
...
Hi.Internal.createDirectives = function(module_name,name,styles,cpcontroller){
    ...
    var main_directiveCreator = new Hi.Internal.DirectiveCreator(module_name,name,main_directive_name,false,cpcontroller);
    ...
    for(var style_name in styles){
        ...
        var directiveCreator = new Hi.Internal.DirectiveCreator(module_name,name,directive_name,style_name,cpcontroller);
...
Hi.UI.partialsInstances = {};

Hi.Internal.registerCpInstance = function(instance){
    var element = instance.getElement();
    var name = $(element).attr("name");

    if(name){

        Hi.UI.partialsInstances[name] = instance;
        Hi.Internal.fireControllerReady(name,instance);

    }

};

Hi.Internal.setPartialReady = function(instance,element){
...
Hi.Internal.postLoad = function(jsFiles,html,noMarkup){
    ...
    if(Array.isArray(jsFiles)||typeof jsFiles ==='object'){
        ...
        for(var jsFile_index in jsFiles){
            ...
            if(typeof js_filename ==='function'){
                ...
                var only_scripts = Hi.Internal.postLoad(js_filename,html,true);
...
Hi.Internal.runPostLoad = function(jsFiles,context){
...
Hi.Internal.preLoad = function(jsFiles,html,context,noMarkup){
    ...
    if(Array.isArray(jsFiles)||typeof jsFiles ==='object'){
        ...
        for(var jsFile_index in jsFiles){
            ...
            if(typeof js_filename ==='function'){
                ...
                var only_scripts = Hi.Internal.preLoad(jsFiles,html,context,true);
...
Hi.Internal.DirectiveCreator= function(module_name,name,directiveName,style_name,cpcontroller){
    ...
    this.create = function(){
        ...
        Hi.UI.ngApp.directive(this.directiveName,function($compile){
            ...
            $directive.compile = function(element,attrs){
                ...
                return  {
                    ...
                    pre : function($scope,element,attrs){
                        ...
                        var partialName = Hi.explode('-',tagName)[0];
                        var cpcontroller = Hi.UI.controllers[partialName];
                        ...
                        if(attrs.name){
                            ...
                            if(_){
                                ...
                                _[instanceName] = $scope;

                            }else{

                                Hi.Internal.$thisInclude(instanceName,$scope);
                    ...
                    post : function($scope,element,attrs){
                        ...
                        Hi.Internal.setPartialReady($scope,element);
...
Hi.Internal.Push = {};
Hi.Internal.Push.init = function(){
...
if(typeof Init !=='undefined'){
    ...
    if(Init.setup){
        ...
        if(Init.setup.modules){

            modulesInjected = Init.setup.modules;

        }

        if(Init.setup.run){

            appRun = Init.setup.run;
...
Hi.UI.setApp(angularApp);


Hi.UI.ngApp.directive('scrollForever',function(){
    ...
    return  {
        ...
        link : function($scope,element,attrs){
            ...
            Hi.Internal.$thisInclude(attrs.name,{$scroll:scrolling});
...
Hi.UI.ngApp.directive('partialLoad',function($compile){
    ...
    return {
        ...
        link : function($scope,element,attrs){
            ...
            if(attrs.src&&attrs.onload){
                ...
                if(Hi.UI.partialViews.hasOwnProperty(attrs.src)){

                    htmlCompiler(Hi.UI.partialViews[attrs.src]);

                }

                $.get(attrs.src,function(html){

                    Hi.UI.partialViews[attrs.src] = html;
                    htmlCompiler(html);
...
Hi.UI.ngApp.directive('ajaxify',function(){
    return  {
...
Hi.UI.ngApp.directive('aload',function(){
...
Hi.UI.ngApp.directive('bgaload',function(){
...
Hi.UI.ngApp.directive('placeholderx',function($compile){
    ...
    return {
        ...
        link: function($scope, element, attributes) {
            ...
            $scope.remake = function(){
                ...
                var compiled__ = compile_function($scope);
                element.html(compiled__);
            };
...
Hi.UI.ngApp.directive('extends',function($compile){
    ...
    if(the_requested_url.startsWith(the_base_url)){
        ...
        if(param.state){
            ...
            if(the_requested_url[the_requested_url.length-1]==='#'){
                ...
                var route = Hi.getLog(the_requested_url);
...
//Loader actual
Hi.UI.ajaxLoader.current = false;

//API para criar um loader
Hi.UI.ajaxLoader.set = function(loader_closure) {
    ...
    new_loader.init = function(){
        ...
        Hi.messages.log('initilializing loader');
    ...
    new_loader.onError = function(err){
        ...
        Hi.messages.error('loder received an error notification');
    ...
    new_loader.onLoad = function(config){
        ...
        Hi.messages.log('showing loader');
    ...
    Hi.UI.ajaxLoader.current=jQuery.extend(new_loader,loader);
...
Hi.ServerGroup = function(route,events){
    ...
    //Path da view
    this.path = Hi.Internal.generateViewPath(route.module,route.model,route.action);
    ...
    this.invokeEvent = function(name,callback,params,pre,post,fail){
        ...
        if(this.events.hasOwnProperty(name)){
            ...
            var http_req_url = Hi.Internal.eventURL(this.route,name);
            ...
            if(!callback){
                ...
                callback = function(data){
                    ...
                    Hi.messages.log("server_response : "+data);
    ...
    this.getStub = function(){
        ...
        for(var eventName in this.events){
            ...
            var default_invoker = new Hi.EventInvoker(eventName,me,false,false);
            var pre_and_post_invoker = new Hi.EventInvoker(eventName,me,false,true);
            var noparams_invoker = new Hi.EventInvoker(eventName,me,false,false,true);
...
//Grupo de eventos
Hi.setServerGroup = function(route,events){

    var serverGroup = new Hi.ServerGroup(route,events);
    var path = Hi.Internal.generateViewPath(route.module,route.model,route.action);
    Hi.serverGroups[path] = serverGroup;
...
Hi.EventInvoker = function(name,group,nocallback,pre_and_post,noparams){
...
Hi.UI.lang = {};
Hi.UI.lang.dict = {};
Hi.UI.lang.getItem = function(key,default_value){

    if((typeof Hi.UI.lang.dict[key] !=undefined) && (Hi.UI.lang.dict[key] != false) && Hi.UI.lang.dict[key]!=undefined  ){

        return Hi.UI.lang.dict[key];
...
function __t(string,key){
    ...
    if(key){
        ...
        return Hi.UI.lang.getItem(key,string);

    }else{

        return Hi.UI.lang.getItem(string);
...
Hi.UI.ngApp.directive('translate',function(){
    return  {
        ...
        link : function($scope,element,attrs){
            ...
            props.forEach(function(name,index){
                ...
                if(propKey!=''){
                    ...
                    var translatedKey = Hi.UI.lang.getItem(propKey.trim(),propValue.trim());
                    ...
                    var translatedValue = Hi.UI.lang.getItem(propValue.trim());
            ...
            if(translateKey!=''){
                ...
                var translatedKey = Hi.UI.lang.getItem(translateKey.trim(),innerHTML.trim());
                ...
                var innerHTML = $(element).html();
                var translatedHtml = Hi.UI.lang.getItem(innerHTML.trim());
...
Hi.app = {};
Hi.module = {};
Hi.module.name = false;

Hi.module.setNextBorderName = function(name){
 ...
 * @param closure
 */
Hi.module.border = function(closure){

    var border_closure = closure;

    if(Hi.module.name){

        Hi.app[Hi.module.name] = border_closure;
...
Hi.getServerGroup = function(path_object_){

    var path_object = Hi.routes.resolve(path_object_);
    var path = Hi.Internal.generateViewPath(path_object.module,path_object.controller,path_object.action);


    //Existe um server group para este path
    if(Hi.serverGroups.hasOwnProperty(path)){

        //Obtem o server group
        var serverGroup = Hi.serverGroups[path];
...
Hi.loadedControllers = [];

Hi.setLoadedController = function(controller, action){

    Hi.loadedControllers.push(controller+"/"+action);

}

Hi.wasControllerLoaded = function(controller,action){

    var url = controller+"/"+action;
    var urlIndex = Hi.loadedControllers.indexOf(url);
    if(urlIndex==-1){
...
function global(name,value){
    ...
    if(typeof value != "undefined"){
        ...
        Hi.globals[name] = value;

    }else{


        if(Hi.globals.hasOwnProperty(name)){

            return Hi.globals[name];
...
Hi.messages = {};
Hi.messages.outputErrors = false;
Hi.messages.outputWarnings = false;
Hi.messages.outputLogs = false;
Hi.messages.outputInfo = false;

Hi.messages.log = function(msg){

    if(Hi.messages.outputLogs){
...
Hi.messages.warn = function(msg){

    if(Hi.messages.outputWarnings){
...
Hi.messages.info = function(msg){

    if(Hi.messages.outputInfo){
...
Hi.messages.error = function(msg){

    if(Hi.messages.outputErrors){
...
Hi.explode = function(glueChar,string){
...
var __flush = Hi.flush;

var FrontierPromisse = function(){
