...
Yapiys.Internal.setNextViewPath = function(module_name,model,action){
        ...
        if(Yapiys.UI.uimaster){
            ...
            for(template_variable_name in Yapiys.UI.uimaster){
                ...
                if(private_index!=-1){
                    ...
                    //Variavel publica
                    _template[template_variable_name] = template_variable_value;
                }
    ...
    context['reload'] = function(html){
        ...
        if(create_controller){
            ...
            after_apply = function(){
                ...
                if(_template.afterViewLoad){


                    if(typeof _template.afterViewLoad ==='function' ){

                        if(typeof Yapiys.Internal.nextViewPath!="undefined"){

                            var resolved_route = Yapiys.routes.resolve(Yapiys.Internal.nextViewPath);
                            _template.afterViewLoad.apply(_,[resolved_route]);
            ...
            if(!Yapiys.Internal.started_up){
                ...
                Yapiys.Internal.Push.init();

                if(_template.startup){


                    if(typeof _template.startup ==='function' ){

                        if(typeof Yapiys.Internal.nextViewPath!="undefined"){

                            var resolved_route = Yapiys.routes.resolve(Yapiys.Internal.nextViewPath);
                            _template.startup.apply(_template,[resolved_route]);
...
Yapiys.Internal.bindParamsAndClosures = function(context,context_variables){
    ...
    for(var root_variable_name in UIRoot){
        ...
        _template[root_variable_name]=root_variable_value;
...
Yapiys.UI.closeView = function(){
    ...
    if(_template){
        ...
        //A view declarou um evento onClose
        if(_template.hasOwnProperty('$onClose')){

            //Executa o onClose
            var onClose = _template['$onClose'];
...
Yapiys.logRoute = function(url){

    Yapiys.history.push(url);
    return Yapiys.history.length -1;

};


Yapiys.isGoingBack = false;

Yapiys.getLog = function(url){

    if(Yapiys.history.length>0){

        var theUrl = Yapiys.history[0];
        Yapiys.history.removeVal(theUrl);
        return theUrl;
...
Yapiys.route = function(route_name_or_object,getParams){
    ...
    Yapiys.routes.last = {name:route_name_or_object};
    ...
    */
    ...
    if(!route_object.dialog){
        ...
        if(Yapiys.wasControllerLoaded(route_object.controller,route_object.action)){
        ...
        Yapiys.Internal.getRoute(route_object,function(server_response){
            ...
            if(server_response.controller) {
                ...
                Yapiys.Internal.setNextClosureInformation(false, route_object.controller, route_object.action);
                eval(server_response.controller);
                Yapiys.setLoadedController(route_object.controller, route_object.action);
            ...
            Yapiys.Internal.setNextViewPath(module_name,controller,action);
            var routeIndex = Yapiys.logRoute(path);
            Yapiys.UI.setLocation(path,JSON.stringify({index:routeIndex}));
...
Yapiys.routeBack = function(route_name_or_object,getParams){

    Yapiys.isGoingBack = true;
    Yapiys.route(route_name_or_object,getParams);
...
Yapiys.Internal.Push = {};
Yapiys.Internal.Push.init = function(){
    ...
    socket.onerror = function(){
        ...
        if(_template.hasOwnProperty("$disconnected")){

            _template.$disconnected.call(_template);
    ...
    socket.onmessage = function(event){
        ...
        try{
            ...
            if(receivedMessage.hasOwnProperty("success")){
                ...
                if(_template.hasOwnProperty("$connected")){

                    _template.$connected.call(_template);
            ...
            if(receivedMessage.hasOwnProperty("type")&&receivedMessage.hasOwnProperty("envelope")){
                ...
                if(_.hasOwnProperty(notificationToken)){
                    ...
                    if(_template.hasOwnProperty(notificationToken)){

                        if(_template[notificationToken].hasOwnProperty(handlerName)){

                            _template[notificationToken][handlerName].call(_template,envelope);
                            return;
                        }
...
Yapiys.loadedControllers = [];

Yapiys.setLoadedController = function(controller, action){

    Yapiys.loadedControllers.push(controller+"/"+action);

}

Yapiys.wasControllerLoaded = function(controller,action){

    var url = controller+"/"+action;
    var urlIndex = Yapiys.loadedControllers.indexOf(url);
    if(urlIndex==-1){
